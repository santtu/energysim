...
class WorldYamlProtocol extends DefaultYamlProtocol {
      ...
      yaml match {
        case YamlArray(array) if array.length >= 1 ⇒
          ...
          (name, params) match {
            ...
            case (typeName, _) ⇒
              deserializationError(s"capacity model ${typeName} not recognized")
          }
...
object Model extends WorldYamlProtocol {
  def fromFile(file: String): World = {
    val data = scala.io.Source.fromFile(file).getLines().mkString("\n")
    println(s"data=$data")
    val yaml = data.parseYaml
    println(s"yaml=$yaml")
    val model = yaml.convertTo[World]
